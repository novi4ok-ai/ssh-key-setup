...
func New(a fyne.App, run Runner) *View {
	...
	v.Host.SetPlaceHolder("192.168.1.10")
	...
	v.Password.SetPlaceHolder("Пароль пользователя на сервере")
	v.Password.Validator = setup.ValidatePassword
	...
	v.Copy.Disable()
	v.Details = paragraph("")
	resultTitle := text("Доступ настроен", 23, accent, true)
	v.result = card(container.NewVBox(resultTitle, paragraph("Подключайтесь из терминала этой командой:"), v.Command, v.Copy))
	v.result.Hide()
	...
	form := card(container.NewVBox(
		...
		container.NewGridWithColumns(2, field("IP-адрес сервера", v.Host), field("Порт SSH", v.Port)),
	...
	stepBox.Add(paragraph("Ed25519 · без парольной фразы\nЗакрытый ключ остаётся на компьютере."))
	...
	actions := container.NewBorder(nil, nil, nil, container.NewHBox(v.Cancel, v.Start), paragraph("Пароль не сохраняется на диск."))
	footer := card(container.NewVBox(v.Status, v.Progress, actions))
	details := widget.NewAccordion(widget.NewAccordionItem("Подробности настройки", v.Details))
	content := container.NewVBox(header, columns, footer, v.result, details)
...
func (v *View) setBusy(busy bool) {
	v.busy = busy
	for _, entry := range []*widget.Entry{v.Host, v.Port, v.User, v.Password} {
		if busy {
			entry.Disable()
		} else {
			entry.Enable()
